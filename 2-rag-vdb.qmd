...
The data are stored in the s3 storage system of the SSPCloud plateform (uploaded from [Eurostat](https://showvoc.op.europa.eu/#/datasets/ESTAT_Statistical_Classification_of_Economic_Activities_in_the_European_Community_Rev._2.1._%28NACE_2.1%29/downloads))
...
- Import NACE data (for example, as a list of dictionnaries with the structure below)
...
- Build a class `NaceDocument` that will help you handling your NACE documents : 
    ...
    - Creates a structured text for each code :
        - example: attribute `text_to_embed`, created by a `to_embedding_text` method, 
        - this text will be ready to be embedded, 
        - Best would be to be parametrize whether to include or not each possible field (e.g : text with or without "Excludes" feature). 

See an example of a cleanned and ready for embedding text : 
...
- Create a text from a single Nace code example, with or without `Exludes` field
...
- Use your `client_qdrant` and the method `recreate_collection` to create an new Qdrant collection (e.g: a new vector database). For example, call it __nace-collection__.
...
- Creates a new method to build a Qdrant `PointStruct` like dictionnary (see below). This structure is required by the Qdrant API.
...
- Recreate your Nace documents, all your embeddings and generate all points (PointStruc).
...
- Upload your points into your Qdrant collection, using `client_qdrant.upsert` method. Try to bach it.
