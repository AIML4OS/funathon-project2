...
This first part will show you how to build a vector database, e.g the "augmented" knowledge aimed to guide your LLM's responses.
...
- 3. Embedded all these texts (1 text => 1 vector point)
- 4. Update the points into Qdrant (the vector database service)
...
::: {.callout-tip collapse="false" icon=false}
## `<i class="bi bi-book"></i>`{=html} Exercice 2: handling NACE data
...
Fine! 
For now, let's choose an embedding model (e.g. a large language model able to transform text to __meaningful__ numerical vector).
...
- Use your `client_qdrant` and the method `recreate_collection` to create a new Qdrant collection (e.g: a new vector database). For example, call it __nace-collection__.
...
- Creates a new method to build a Qdrant `PointStruct` like dictionary (see below). This structure is required by the Qdrant API.
