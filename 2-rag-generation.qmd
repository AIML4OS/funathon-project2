...
This second tutorial builds on the vector database created in part 1. It completes the RAG pipeline by loading labeled data, retrieving context from Qdrant, and generating final answers from a query. You will learn how to **automatically classify free-text activity labels** into the NACE 2.1 nomenclature, and then **evaluate the quality of the pipeline** at each stage.
...
2. Call [client_llmlab.chat.completions.create](https://developers.openai.com/api/reference/python/resources/chat/subresources/completions/methods/create) with the system and user messages.
...
results[["activity", "true_code", "pred_code", "retriever_hit", "pipeline_correct"]].head()
```
...
```{python}
#| label: retriever-accuracy-hint
#| echo: false
#| output: true

results["retriever_hit"].value_counts().rename({True: "Retriever found true code", False: "Retriever missed"})
```
...
```{python}
#| label: llm-accuracy-hint
#| echo: false
#| output: true

results[results["retriever_hit"]]["pipeline_correct"].value_counts().rename({True: "LLM correct", False: "LLM wrong"})
```
...
```{python}
#| label: pipeline-accuracy-hint
#| echo: false
#| output: true

results["pipeline_correct"].value_counts().rename({True: "Correctly coded", False: "Incorrectly coded"})
```
